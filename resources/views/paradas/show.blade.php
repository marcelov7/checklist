            ...
            <div class="card">
                <div class="card-header bg-primary text-white d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                    ...
                    <div class="d-flex flex-column flex-md-row align-items-stretch align-items-md-center gap-2 gap-md-3 w-100 w-md-auto">
                        ...
                        <span class="badge bg-light text-dark fs-6">{{ $percentualGeral }}% Completo</span>
                    </div>
                </div>
                <div class="card-body">
                    <div class="row">
        ...
        <div class="row mb-4">
            <div class="col-12">
                <div class="card area-card shadow-sm">
                    <!-- Header da Área -->
                    <div class="card-header bg-info text-white" data-area-id="{{ $area->id }}">
                        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-2 gap-md-0">
                            ...
                            <div class="d-flex align-items-center gap-2 gap-md-3 w-100 w-md-auto justify-content-between justify-content-md-end">
                                ...
                                <span class="badge bg-light text-dark fs-7 fs-md-6">{{ $percentualPorArea[$area->id] ?? 0 }}% Completo</span>
                            </div>
                        </div>
                    </div>
                                                    ...
                                                    <div class="checklist-content">
                                                        ...
                                                        <div class="checklist-actions">
                                                            ...
                                                            @if($statusAtual === 'pendente')
                                                                ...
                                                                @else
                                                                    <button class="btn btn-success btn-sm" onclick="marcarOK('{{ $key }}', {{ $teste->id }}, event); return false;">
                                                                        <i class="fas fa-check me-1"></i>OK
                                                                    </button>
                                                                    <button class="btn btn-danger btn-sm" onclick="reportarProblema('{{ $key }}', {{ $teste->id }}, event); return false;">
                                                                        <i class="fas fa-exclamation-triangle me-1"></i>Problema
                                                                    </button>
                                                                    <button class="btn btn-info btn-sm" onclick="marcarNaoAplica('{{ $key }}', {{ $teste->id }}, event); return false;">
                                                                ...
                                                                <div class="status-badge">
                                                                    ...
                                                                    @else
                                                                        @if(empty($item['resolucao']))
                                                                            <button class="btn btn-warning btn-sm" onclick="mostrarResolucao('{{ $key }}', {{ $teste->id }})">
                                                                                <i class="fas fa-wrench me-1"></i>Resolver Problema
                                                                            </button>
                                                                        @else
                                                                            <button class="btn btn-success btn-sm" onclick="mostrarResolucao('{{ $key }}', {{ $teste->id }})">
                                                                                <i class="fas fa-edit me-1"></i>Editar Resolução
                                                                            </button>
                                                                        @endif
                                                                        <button class="btn btn-outline-secondary btn-sm" onclick="editarStatus('{{ $key }}', {{ $teste->id }}, '{{ $statusAtual }}')" title="Alterar status">
...
const paradaFinalizada = {{ $paradaFinalizada ? 'true' : 'false' }};
...
function getCollapseStorageKey() {
    return `collapse_state_parada_{{ $parada->id }}`;
}
...
function finalizarParada() {
    if (confirm('Tem certeza que deseja finalizar esta parada?\n\nApós finalizada, não será mais possível fazer alterações nos checklists.')) {
        ...
        fetch(`{{ route('paradas.finalizar', $parada) }}`, {
            method: 'POST',
            body: formData,
            headers: {
...
function reabrirParada() {
    if (confirm('Tem certeza que deseja reabrir esta parada?\n\nApós reaberta, será possível fazer alterações nos checklists novamente.\n\nEsta ação só pode ser realizada por administradores.')) {
        ...
        fetch(`{{ route('paradas.reabrir', $parada) }}`, {
            method: 'POST',
            body: formData,
            headers: {
                'Accept': 'application/json',
